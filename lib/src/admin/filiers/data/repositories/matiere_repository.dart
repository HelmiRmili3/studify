...
class MatiereRepository extends StorageRepository {
  final FirebaseFirestore _firestore = FirebaseFirestore.instance;

  /// Fetch a stream of all `Matiere` objects
  Stream<List<Matiere>> fetchMatieres(String niveau) {
    return _firestore
        .collection(Firestore.years)
        .doc('2024')
        .collection(Firestore.matieres)
  ...
  Future<void> addMatiere(Matiere matiere) async {
    try {
      ...
      // Upload the image using StorageRepository functionality
      FileEntity file = await uploadFile(imageFile, 'matieres');
  ...
  /// Save the Matiere to Firestore
  Future<void> _saveMatiereToFirestore(
      Matiere matiere, FileEntity filedata) async {
    await _firestore
        .collection(Firestore.years)
        .doc('2024')
        .collection('matieres')
